module FML
  ...
  class RequiredIfValidation
    ...
    def validate
      # if parent is true, child must be truthy (is that the right semantics?)
      if @parent.value && !@child.value
        raise DependencyError.new(<<-EOM, @child.name, @parent.name)
  ...
  class MinLengthValidation
    ...
    def validate
      ...
      if @field.value && @field.value.length < @minlength
        raise ValidationError.new(<<-EOM, @field.name)
